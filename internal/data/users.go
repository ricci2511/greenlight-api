...
// Checks whether the provided plaintext password matches the hashed password stored in the password struct.
...
func ValidateUser(v *validator.Validator, user *User) {
	...
	// If this happens its probably an error from our side because we forgot to set a password for the user.
	// So for safety we panic here.
...
func (m UserModel) Insert(user *User) error {
	...
	// Explicitly check if the provided email violates the UNIQUE constraint.
	if err != nil {
...
func (m UserModel) GetForToken(scope, tokenPlaintext string) (*User, error) {
	...
	// 32 bytes token hash array is converted to a slice since the pq driver does not support arrays.
